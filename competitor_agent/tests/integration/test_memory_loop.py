...
class TestMemoryLoop:
    def test_analysis_sediments_skills_and_source_outcomes(self, fake_extractor, memory) -> None:
        api = CompetitorAnalysisAPI(extractor=fake_extractor, use_llm=False, memory=memory, max_iterations=10)
    ...
    def test_memory_persists_across_reload(self, tmp_path, fake_extractor) -> None:
        data_dir = tmp_path / "persist"

        mem = FourLayerMemory(data_dir)
        api = CompetitorAnalysisAPI(extractor=fake_extractor, use_llm=False, memory=mem, max_iterations=10)
        api.analyze("分析 Cursor", mode="single")

        reloaded = FourLayerMemory(data_dir)
        assert reloaded.retrieve_skills("cursor"), "记忆应落盘并可重新加载"

    def test_second_plan_hits_memory_boost(self, fake_extractor, memory) -> None:
        api = CompetitorAnalysisAPI(extractor=fake_extractor, use_llm=False, memory=memory, max_iterations=10)
        api.analyze("分析 Cursor", mode="single")
