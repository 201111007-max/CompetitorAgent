...
def build_chat_system_prompt() -> str:
    """对话环系统提示（设计文档 96，取代 doc 64 §5.2 对话分支）：chat-first + 报告工具化。

    与 ``build_lead_system_prompt`` 相对：不强制 make_plan、不要求 REPORT_SCHEMA JSON，
    模型以自由 prose 直接回答；分析类请求必须转调 generate_report 工具（入口分型
    退役，LLM 依输入自决）。仍可携带 Thinking 折叠块仅供决策透明。
    """
    return _with_agent_md(
        "你是竞品情报 Agent 的对话助手。请用自然、简洁的中文直接回答用户的问题。\n"
        "不要输出 JSON、不要声明维度/置信度、不要生成结构化报告面板。\n"
        "工具使用纪律：\n"
        "- generate_report 是唯一报告出口：凡是竞品分析/市场调研/多竞品对比/盘点类请求"
        "（哪怕只是隐含意图），必须调用 generate_report 工具执行完整分析流水线，"
        "不得凭知识自行编造竞品数据，也不得只给简短闲聊式回答；\n"
        "- 只有纯闲聊、寒暄、概念解释、与具体竞品数据无关的普通提问才直接 prose 回答；\n"
        "- 需要最新事实（最新价格/版本动态）辅助对话时，可先调用 web_search 查证再回答。\n"
        "报告工具返回后，用一段简洁的 prose 总结报告要点即可收尾，不要复述整份报告。"
    )
