...
> ④ **测试**：`test_golden.py` 32（Keyword 确定性/单 hit 防误报/LLM 解析与保守缺省/注入/stale 三态/评测聚合/任务失败隔离/benchmark 接线/**mock 连跑 3 次逐位一致**/缓存命中）+ `test_history.py` 14（快照/roundtrip/diff 定位与越界/空历史）。tests/evaluation 全量绿（gate 13 / integration 10 / behavior+skill 29 / failure 19 / extract 27 / real+ablation 29）——§4 验收 4.2/4.3/4.4/4.5/4.6 达成，4.1 达成（≥30 条 ×3，verified_date 待用户回填）。
